...
![Não codifique como um chefe 👨‍🍳 italiano! Macarronada só é saborosa no seu prato! 😋](pasta_00.png){fig-align="left" width="350"}

::: {layout="[25,-2,10]"}
![Quando termino de ajudar](pasta_face.gif){width="30%"}
![Quando alguém me pede ajuda e me apresenta um código bagunçado. 😶](desconfiado.gif){width="40%"}
:::
...
#| echo: false
#| warning: false
#| fig-cap: "Note que temos uma ótima aproximação de $f(x)$, em torno de $a = 0$, por meio do polinômio de Taylor, considerando $N = 2$."
library(ggplot2)
library(purrr)
library(glue)
...
1.  A função `taylor(n = 1L, func, x, a = 0,...)` que recebe o número de somas `n`, a função `func` a ser aproximada, um valor `x` e o valor de `a`. Também é possível passar argumentos adicionais usando o operador **varargs** dot-dot-dot (`...`), em que você poderá controlar argumentos da função `Deriv()` do pacote [**Deriv**](https://github.com/sgsokol/Deriv) utilizado para o cálculo das derivadas simbólicas de $f(x)$. O pacote encontra-se no *Comprehensive R Archive Network* - CRAN e poderá ser instalado fazendo `install.packages("Deriv")`[@Deriv];
